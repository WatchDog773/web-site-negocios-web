...
//Mostrar todos los cursos si el usuario no esta autenticado
exports.listaCursos = async (req, res, next) => {
  const usuario = res.locals.usuario;
  ...
  try {
    ...
    res.render("lista_cursos", { cursosArrayView, usuario, verifyAuth });
  } catch (error) {
    mensajes.push({
      mensaje: "No se han logrado cargar los datos",
      type: "alert-danger",
    });
    res.render("lista_cursos", { mensajes, usuario, verifyAuth });
  }
};
// Buscar curso
